 ...
 * @since 3.1
 */
public abstract class YJFCacheAspectSupport implements InitializingBean {
	...
	private static final String			CACHEABLE		= "cacheable", UPDATE = "cacheupdate",
			EVICT = "cacheevict";
	...
	public void setCacheOperationSources(CacheOperationSource... cacheOperationSources) {
		...
		this.cacheOperationSource = (cacheOperationSources.length > 1 ? new CompositeCacheOperationSource(
			cacheOperationSources) : cacheOperationSources[0]);
	}
	...
	protected CacheOperationContext getOperationContext(CacheOperation operation, Method method,
	...
	protected Object execute(Invoker invoker, Object target, Method method, Object[] args) {
		...
		final Collection<CacheOperation> cacheOp = getCacheOperationSource().getCacheOperations(
			method, targetClass);
		
		// analyze caching information
		if (!CollectionUtils.isEmpty(cacheOp)) {
			Map<String, Collection<CacheOperationContext>> ops = createOperationContext(cacheOp,
	...
	public abstract boolean getWebServiceEnable();
							...
							if (evictOp.isCacheWide()) {
								...
								if (log) {
									logger.trace("Invalidating entire cache for operation "
													+ evictOp + " on method " + context.method);
								}
							} else {
								// check key
								if (key == null) {
								...
								if (log) {
									logger.trace("Invalidating cache key " + key
													+ " for operation " + evictOp + " on method "
													+ context.method);
								}
								cache.evict(key);
							}
	...
	private static class CacheStatus {
		...
		CacheStatus(Map<CacheOperationContext, Object> cUpdates, boolean updateRequired,
					Object retVal) {
	...
	private Map<String, Collection<CacheOperationContext>> createOperationContext(	Collection<CacheOperation> cacheOp,
		...
		for (CacheOperation cacheOperation : cacheOp) {
			CacheOperationContext opContext = getOperationContext(cacheOperation, method, args,
				target, targetClass);
	...
	protected class CacheOperationContext {
		...
		public CacheOperationContext(CacheOperation operation, Method method, Object[] args,
			...
			this.caches = YJFCacheAspectSupport.this.getCaches(operation);
			this.target = target;
			this.method = method;
			this.args = args;
			
			this.evalContext = evaluator.createEvaluationContext(caches, method, args, target,
				targetClass);
		}
