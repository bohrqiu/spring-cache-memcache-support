...
import org.springframework.cache.interceptor.YJFCacheInterceptor;
import org.w3c.dom.Element;

import com.yjf.common.cache.YJFSpringCacheAnnotationParser;
 ...
 * @since 3.1
 */
class YJFAnnotationDrivenCacheBeanDefinitionParser implements BeanDefinitionParser {
	
	private static final Logger	logger	= LoggerFactory
											.getLogger(YJFAnnotationDrivenCacheBeanDefinitionParser.class
												.getName());
	...
	private static void parseCacheManagerProperty(Element element, BeanDefinition def) {
		def.getPropertyValues().add("cacheManager",
			new RuntimeBeanReference(YJFCacheNamespaceHandler.extractCacheManager(element)));
	}
	...
	private static class AopAutoProxyConfigurer {
		...
		public static void configureAutoProxyCreator(Element element, ParserContext parserContext) {
			...
			if (!parserContext.getRegistry().containsBeanDefinition(CACHE_ADVISOR_BEAN_NAME)) {
				...
				//使用YJFSpringCacheAnnotationParser来解析
				ConstructorArgumentValues constructorArgumentValues = new ConstructorArgumentValues();
				constructorArgumentValues
					.addGenericArgumentValue(new YJFSpringCacheAnnotationParser());
				sourceDef.setConstructorArgumentValues(constructorArgumentValues);
				String sourceName = parserContext.getReaderContext().registerWithGeneratedName(
					sourceDef);
				
				// Create the CacheInterceptor definition.
				//使用YJFCacheInterceptor
				RootBeanDefinition interceptorDef = new RootBeanDefinition(
					YJFCacheInterceptor.class);
				interceptorDef.setSource(eleSource);
				interceptorDef.setRole(BeanDefinition.ROLE_INFRASTRUCTURE);
				parseWebServiceResultOptimize(element, interceptorDef);
				parseCacheManagerProperty(element, interceptorDef);
				YJFCacheNamespaceHandler.parseKeyGenerator(element, interceptorDef);
				...
				parserContext.getRegistry().registerBeanDefinition(CACHE_ADVISOR_BEAN_NAME,
					advisorDef);
		...
		private static void parseWebServiceResultOptimize(Element element,
															RootBeanDefinition interceptorDef) {
			String enable = element.getAttribute("webservice-result-optimize-enable");
			if (Boolean.parseBoolean(enable)) {
				interceptorDef.getPropertyValues().add("webServiceEnable", true);
				logger.info("启用webservice结果缓存优化支持");
			}
		}
	}
}
